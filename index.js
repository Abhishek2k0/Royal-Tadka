...
function openWhatsapp() {
  if ($("#address")[0].value === "") {
    ...
    for (var i = 0; i < food.length; i++) {
      ...
      itemName = itemName + name + " x " +quantity +" ----> " + food[i][2]*food[i][1]+" \n" ;
    }
    ...
    // Assuming you have a server-side endpoint to handle sheeddb.io API
    let sheeddbApiUrl = 'https://sheetdb.io/api/v1/dn8zsedf8wbb6';

    // Prepare the data object to be sent to the sheeddb.io API
    let dataToSend = {
      data: {
        Name: address,
        DishName: itemName,  // This assumes 'wTxt' contains the item names and quantities
        Amount: total,
        Notes: note,
    ...
    // Use AJAX to send data to sheeddb.io API
    $.ajax({
      type: 'POST',
      url: sheeddbApiUrl,
      ...
      success: function(response) {
        ...
        console.log('Data sent to sheeddb.io API successfully');
        // If you want to open WhatsApp after sending data to Google Sheets, you can do it here
        alert('Order successfully submitted!');

      },
      error: function(error) {
        // Handle error if needed
        console.error('Error sending data to sheeddb.io API:', error);
      }
    });
  }
}
